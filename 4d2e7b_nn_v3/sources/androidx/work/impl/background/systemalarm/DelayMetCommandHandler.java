...
public class DelayMetCommandHandler implements WorkConstraintsCallback, ExecutionListener, WorkTimer.TimeLimitExceededListener {
    ...
    DelayMetCommandHandler(@NonNull Context context, int i10, @NonNull String str, @NonNull SystemAlarmDispatcher systemAlarmDispatcher) {
        ...
        synchronized (this.mLock) {
            try {
                if (this.mCurrentState < 2) {
                    ...
                    systemAlarmDispatcher.postOnMainThread(new SystemAlarmDispatcher.b(systemAlarmDispatcher, createStopWorkIntent, this.mStartId));
                    if (this.mDispatcher.getProcessor().isEnqueued(this.mWorkSpecId)) {
                        ...
                        systemAlarmDispatcher2.postOnMainThread(new SystemAlarmDispatcher.b(systemAlarmDispatcher2, createScheduleWorkIntent, this.mStartId));
                    } else {
    ...
    public void onExecuted(@NonNull String str, boolean z10) {
        Logger.get().debug(TAG, String.format("onExecuted %s, %s", new Object[]{str, Boolean.valueOf(z10)}), new Throwable[0]);
        cleanUp();
        if (z10) {
            Intent createScheduleWorkIntent = CommandHandler.createScheduleWorkIntent(this.mContext, this.mWorkSpecId);
            SystemAlarmDispatcher systemAlarmDispatcher = this.mDispatcher;
            systemAlarmDispatcher.postOnMainThread(new SystemAlarmDispatcher.b(systemAlarmDispatcher, createScheduleWorkIntent, this.mStartId));
        }
        if (this.mHasConstraints) {
            Intent createConstraintsChangedIntent = CommandHandler.createConstraintsChangedIntent(this.mContext);
            SystemAlarmDispatcher systemAlarmDispatcher2 = this.mDispatcher;
            systemAlarmDispatcher2.postOnMainThread(new SystemAlarmDispatcher.b(systemAlarmDispatcher2, createConstraintsChangedIntent, this.mStartId));
        }
    }
